#!/usr/bin/env python3
"""
👾 Byte v3.0 — Local AI Assistant
"""
import sys, os, argparse, time, subprocess, json, datetime, re
...
if sys.platform=="win32":
    ...
    def getch():
        ...
        try:
            ...
            if ch=='\x1b':
                sys.stdin.read(1); ch3=sys.stdin.read(1)
...
# ── Menu engine ───────────────────────────────────────────────────────────────
def draw_menu(items, sel, header_lines=None, search=""):
    clear(); pad()
    print(f"  {bold('👾 BYTE')}  {dim('v'+VERSION)}  {tag('AI')}")
    print(f"  {line()}")
    if header_lines:
        for hl in header_lines: print(f"  {hl}")
        print(f"  {line('·')}")
    pad()
    for i,item in enumerate(items):
        label=item[0]; desc=item[1] if len(item)>1 else ""; icon=item[2] if len(item)>2 else " "
        if i==sel:
            pre=c(C.CYAN+C.BOLD," ❯ "); lbl=f"{C.BOLD}{C.WHITE}{label}{R}"
            dsc=f"  {C.CYAN}{desc}{R}" if desc else ""; ic=c(C.CYAN,icon)
        else:
            pre="   "; lbl=c(C.WHITE,label)
            dsc=f"  {dim(desc)}" if desc else ""; ic=dim(icon)
        print(f"  {pre}{ic}  {lbl}{dsc}")
    pad()
    if search: print(f"  {dim('search:')} {hi(search)}")
    print(f"  {dim('↑↓ move  enter select  esc back  / search  q quit')}")
    pad()

def run_menu(title, items, header_lines=None, start=0):
...
def _wait_key(msg="  Press any key..."):
...
def _spinner(label,fn):
    ...
    import threading; t=threading.Thread(target=worker,daemon=True); t.start()
    i=0
    while not done[0]:
...
def _do_pull(name):
    ...
    try:
        proc=subprocess.Popen([cfg.get("ollama_path","ollama"),"pull",name],
            stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True,encoding="utf-8",errors="replace")
        for ln in proc.stdout:
            ln=ln.rstrip()
            if ln: print(f"  {dim(ln)}", end="\r" if "%" in ln else "\n", flush=True)
        proc.wait(); print()
...
""")
    ...
    # Step 2 – Ollama
    clear(); _ob_hdr(2,3,"Ollama Check")
    ...
    # Step 3 – Model
    clear(); _ob_hdr(3,3,"Choose Model")
    models=list_local_models()
    if models:
        print(f"  {ok(f'Found {len(models)} model(s)!')}\n")
        items=[(m,"","◆") for m in models]+[("Skip","","·")]
        idx,_=run_menu("Choose Model",items,header_lines=[f"  Active: {hi(cfg.get('model','?'))}"])
        if 0<=idx<len(models): cfg["model"]=models[idx]; save_config(cfg)
    else:
        print(f"  {warn('No local models found.')}\n")
        suggestions=[("qwen3:4b","fast & smart"),("llama3.2:3b","lightweight"),
                     ("phi4:latest","compact smart"),("mistral:7b","balanced")]
        for nm,ds in suggestions: print(f"    {hi('◆')}  {bold(nm):<24}{dim(ds)}")
        print()
        pull=input(f"  {hi('Pull a model now?')} {dim('(name or skip)')} › ").strip()
...
def main_menu():
    ...
    idx,_=run_menu("Main",items,header_lines=header)
    dispatch={0:run_chat,1:quick_ask,2:model_menu,3:pull_menu,4:delete_model_menu,
              5:launch_web,6:integrations_menu,7:settings_menu,8:system_info,
              9:show_help,10:_reset_onboard,11:_quit}
    if idx==-1: _quit()
...
def model_menu():
    ...
    if not models:
        _flash("No local models found.",C.YELLOW); return main_menu()
    items=[(m,ok("active") if m==cur else "","◆") for m in models]+[("← Back","",  " ")]
    idx,_=run_menu("Select Model",items,header_lines=[f"  Current: {hi(cur)}"])
...
def delete_model_menu():
    ...
    idx,_=run_menu("Delete Model",items,header_lines=[warn("  This removes the model from disk")])
    if idx==-1 or idx==len(models): return main_menu()
...
def integrations_menu():
    ...
    items.append(("← Back","","  "))
    idx,_=run_menu("Integrations",items,header_lines=[dim("  Configure messaging")])
    if idx==-1 or idx==len(fields): return main_menu()
    fkey=fields[idx][0]; label=fields[idx][1]
    clear(); print(f"\n  {bold(label)}\n  {line()}\n")
    val=input(f"  {hi('New value')} {dim('(enter to keep)')} › ").strip()
    if val: keys[fkey]=val; cfg["api_keys"]=keys; save_config(cfg); _flash(f"✓ {label} saved.")
    integrations_menu()
...
def settings_menu():
    ...
    cfg=load_config()
    editable=[
        ("bot_name","Bot name","👾"),("user_name","Your name","👤"),("emoji","Bot emoji","✨"),
        ("model","Active model","◆"),("ollama_host","Ollama host","🔗"),
        ("ollama_path","Ollama binary","📁"),("port","Web port","🌐"),
        ("theme","Theme","🎨"),("chrome_path","Chrome path","🌍"),
        ("auto_start_ollama","Auto-start Ollama","⚡"),("system_prompt","System prompt","🧠"),
    ]
    ...
    if isinstance(cur,bool):
        r=input(f"  {hi('true/false')} › ").strip().lower(); nv=r=="true"
    elif isinstance(cur,int):
        r=input(f"  {hi('Number')} › ").strip(); nv=int(r) if r.isdigit() else cur
    else:
        nv=input(f"  {hi('New value')} {dim('(enter to keep)')} › ").strip() or cur
    cfg[key]=nv; save_config(cfg); _flash(f"✓ {label} updated.")
    settings_menu()
...
def system_info():
    ...
    if models:
        print(f"\n  {bold('Local Models')}\n  {line('·')}")
        cur=cfg.get("model","")
        for m in models: print(f"    {c(C.CYAN,'◆')}  {m}{ok('  ← active') if m==cur else ''}")
...
def quick_ask():
    ...
    if not ollama_running(): _spinner("Starting Ollama",start_ollama)
    print(f"\n  {c(C.GREEN,'👾')} ",end="",flush=True)
    try:
        for tok in chat_stream([{"role":"user","content":q}]):
            print(tok,end="",flush=True)
    except KeyboardInterrupt: print(warn(" [stopped]"),end="")
    print("\n"); _wait_key(); main_menu()
...
def run_chat(initial_model=None):
    ...
    while True:
        ...
        if user_input.startswith("/"):
            pts=user_input.split(None,1); cmd=pts[0].lower(); arg=pts[1] if len(pts)>1 else ""
            if cmd in("/bye","/exit","/quit","exit","quit"):
                ...
                print(f"\n  {bold('Local models:')}")
                for m in ml: print(f"    {c(C.CYAN,'◆')}  {m}{ok('  ← active') if m==cur else ''}")
                print()
            elif cmd=="/model":
                if arg:
                ...
                if arg: cfg["system_prompt"]=arg; save_config(cfg); print(f"\n  {ok('✓')} Prompt updated.\n")
...
def _do_stream(cfg,history,chat_stream,run_actions,user_input):
    ...
    emoji=cfg.get("emoji","👾")
    print(f"\n  {c(C.GREEN,emoji)} ",end="",flush=True)
    tokens=[]
    try:
        for tok in chat_stream(history): print(tok,end="",flush=True); tokens.append(tok)
    except KeyboardInterrupt: print(warn(" [stopped]"),end="")
    full="".join(tokens); history.append({"role":"assistant","content":full})
...
def _chat_hdr(cfg):
    ...
    print(f"  {line()}")
    print(f"  {dim('/help  /model  /models  /clear  /save  /export  /copy  /web  /menu  /bye')}")
    print(f"  {line()}\n")
...
def _save_chat(history):
    ...
    with open(path,"w",encoding="utf-8") as f:
        ...
        for m in history: f.write(f"[{m['role'].upper()}]\n{m['content']}\n\n")
    ...
    with open(saves[idx],encoding="utf-8") as f:
        for ln in f:
            ...
            if ln in("[USER]","[ASSISTANT]"):
                if role and buf: nh.append({"role":role.lower(),"content":"\n".join(buf).strip()})
                role=ln[1:-1]; buf=[]
        ...
        if m["role"]=="assistant":
            try:
                if sys.platform=="win32":
                    ...
                    subprocess.run(["xclip","-selection","clipboard"],input=m["content"].encode(),check=True)
                print(f"\n  {ok('✓')} Copied.\n")
